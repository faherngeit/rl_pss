...
%==========================================================================
function CurrentProcess (current)
    
    t = current.Time; %здание массива времени
    i_ideal = current.Ideal; %здание массива этал. тока
    i_meas = current.Meas;  %здание массива изм. тока
        ...
        if Amplitude2 > Th1 && Amplitude1 > Th1 %Проверка тока КЗ
            ...
            for h3 = p:length(i_meas) 
                ...
                if DENSITY_VAR <= Th2 %условие возникновения насыщения
                   ...
                   %-------------------------------------------------------
                   Prognoz = prognoz (t, p, L, i_filter, CTParam);
%                    tic
                   %-------------------------------------------------------
                   %вызов функции поиска начальной индукции
                   %-------------------------------------------------------
                   [Initial, h5, i_reconstrac] = initial (p, L, i_meas,...
...
function NoisyCurrent = nois (i_meas)
    for h = 1:length(i_meas)
        NoisyCurrent (h,1) = i_meas(h) * (1 + 0.03 * (2 * rand(1,1)-1)) + ...
            (2 * rand(1,1)-1) * 0.03 * max(i_meas);
    end
end
...
%       Prognoz = z(1,1) * t(p-K+1:p-K+3).^2 + z(2,1) * t(p-K+1:p-K+3) + z(3,1);
      ...
      A = [sin(100*pi * t(p-K-11:p-K-2)) cos(100*pi * t(p-K-11:p-K-2))];
                        
      b = i_filter(p-K-11:p-K-2);
