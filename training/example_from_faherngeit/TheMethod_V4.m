...
%==========================================================================
function [Reconstracted, BuffrNext, options, SS] = TheMethod_V4(CurrentSat, Buffer, options, SS)
%     clc
    WndSize = 20;
    BfrSize = 3 * WndSize;
   ...
   if options.SAT2 == 1
      ...
      %-------------------------------------------------------
      [p, i_reconstr, options, SS] = compensate (i_meas, i_reconstr, 60, 60,...
          CTParam, options, SS);
      Reconstracted = i_reconstr(end-WndSize+1:end);
      
      BuffrNext = i_reconstr(end-BfrSize+1:end);
      i_meas = i_reconstr;
    ...
    for h2  = p:length (i_meas)
        ...
        Th1 = CTParam.I_rated * sqrt(2) * 4 / nt; %Уставка маскимального режима 
...
%             p = p + L; %Время задержки детектора
            ...
            if DENSITY_VAR == 1 %условие возникновения насыщения
               ...
               %вызов функции восстановления тока
               %-------------------------------------------------------
               [p, i_reconstr, options, SS] = compensate (i_meas,...
                       i_reconstr, h5, p, CTParam, options, SS);
   
            end %if DENSITY_VAR

               p = p + 1; %обновление номера счетчика
            
               if options.SAT1 == 0 || p >= length(i_meas) || DENSITY_VAR == 0%определение конца массива данных
                    Reconstracted = i_reconstr(end-WndSize+1:end);
...
function CTParam = ctParam ()
    ...
    CTParam = struct('s', 19.1e-4, 'l', 90e-2, 'w2', 239, 'w1', 2, 'I_rated',...
        600, 'R2', 0.24, 'Rn', 0.96, 'L2', 0, 'Ln', 0, 'N', 80, 'dt',...
...
%         sat1 = sat2;
%         sat2 = sat3;
%         sat3 = 1e5 * var(Flux1(k-j+12:k)/max(abs(Flux1(k-j+12:k))));

% %         State = Flux(k-j+12);
% %         if State < 0 && abs(State) < 1
% %             V = Flux(k-j+12:k) - abs(1 - min(Flux(k-j+12:k)));
% %         elseif State > 0 && State < 1
% %             V = Flux(k-j+12:k) + abs(1 - max(Flux(k-j+12:k)));
% %         else
% %             V = Flux(k-j+12:k);
% %         end
% %         sat3 = 1e5 * var(V/max(abs(V)));
% % 
% % %         if sat3 < Th_B2%Sum_of_current_difference(k-j+1) > Th_i2 && 
           for i = k:length(Flux1)
               DNS = var(Flux1(i-12:i));
               if DNS < 0.0005
                DENSITY_VAR = 1;
                p = i - 14;
                break
...
function Prognoz = prognoz (p, i_filter, CTParam)
      ...
      IPT = 8; %этот параметр определяет длину интервала правильной трансформации
      t = (p-IPT:p-1)'.* CTParam.dt;% с помощью этой строки индексация преобразуется в временной ряд
            
      A = [sin(100*pi * t) cos(100*pi * t)];
                        
      b = i_filter(p-IPT:p-1);
...
function [options, h5, i_reconstr] = initial (options, p,...
        ...
        for h4 = 1:600
            ...
            options.Initial = 1.5 + (h4-1)*0.001; 
            ...
            error1 = error;
            error = norm(resudal(2:end));
            V = sum(abs(i_curve(end))) > sum(abs(Prognoz(end)));
            if error < 10 && V == 1
                i_reconstr (p+1:p+3,1) = Prognoz;
                break
            elseif error1 < error && error > 10  && V == 1
                i_reconstr (p+1:p+3,1) = Prognoz;
                break
            end
        end%h4           
...
%==========================================================================
function [p, i_reconstr, options, SS] = compensate (i_meas, i_reconstr, h5, p,...
                                    CTParam, options, SS)
    Compensated = (i_meas);
    if options.SAT3 == 1
        i_meas(h5) = SS;
